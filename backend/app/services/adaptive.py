...
def select_next_question(db: Session, user_id: str, use_ai: bool = True,
    ...
    if use_ai and use_targeted and random.random() < 0.4:
        try:
            ...
            if has_weaknesses:
                logger.debug("Generating targeted question for %s", weakness_profile.get('recommended_focus'))
                ...
                logger.debug("No weakness data, using standard selection")

        except Exception as e:
            logger.warning("Targeted generation failed: %s, falling back to pool", str(e))
