...
def get_specialty_breakdown(user_id: str, db: Session = Depends(get_db)):
    ...
    # Single query to get all specialty stats using CASE expressions
    # This replaces 8 separate queries with 1 query
    stats = db.query(
        Question.specialty,
        func.count(QuestionAttempt.id).label('total'),
        func.sum(func.cast(QuestionAttempt.is_correct, Integer)).label('correct'),
        func.sum(func.coalesce(Question.recency_weight, 0.5)).label('total_weight'),
        func.sum(
            case(
                (QuestionAttempt.is_correct == True, func.coalesce(Question.recency_weight, 0.5)),
                else_=0
            )
        ).label('weighted_correct')
    ).join(
        Question, QuestionAttempt.question_id == Question.id
    ).filter(
        QuestionAttempt.user_id == user_id,
        Question.specialty.isnot(None)
    ).group_by(
        Question.specialty
    ).all()

    # Build lookup dict from query results
    stats_lookup = {s.specialty: s for s in stats}

    # Build breakdown with all specialties (including those with 0 attempts)
    breakdown = {}
    for specialty in specialties:
        stat = stats_lookup.get(specialty)
        ...
        if stat and stat.total > 0:
            ...
            total_weight = stat.total_weight or 0.5
            weighted_correct = stat.weighted_correct or 0
