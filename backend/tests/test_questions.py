...
class TestQuestionsEndpoints:
    ...
    @pytest.mark.api
    def test_get_question_by_id(self, client: TestClient, test_question: Question):
...
class TestQuestionFiltering:
    ...
    @pytest.mark.api
    def test_filter_by_specialty(self, client: TestClient, test_questions_batch: list[Question]):
        """Test filtering questions by specialty"""
        response = client.get("/api/questions/random?specialty=Internal Medicine")
        # This may or may not return a question depending on generation
        assert response.status_code in [200, 404]

    @pytest.mark.api
    def test_question_has_required_fields(self, client: TestClient, test_question: Question):
